import pandas as pd
from .Codigos_PSI import TECHNI15D, ABDOMI15P, MEDIC2R, SURGI2R
...
def PSI15(df, d_cols, p_cols, death, elective, acute,
    ...
    proc_abd = df[p_cols].isin(ABDOMI15P).any(1)

    string = (f"({age_col}>=18) & "
              f"(({ms_col} in @SURGI2R) | ({ms_col} in @MEDIC2R)) &"
              "@proc_abd")
    psi_denom = df.eval(string)
    ...
    # TODO: Second abdominopelvic procedure >= 1 day after a abdominopelvic
    # procedure
