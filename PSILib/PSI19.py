import pandas as pd
from .Codigos_PSI import DELOCMD, OBTRAID, VAGDELP, INSTRIP


def PSI19(df, d_cols, p_cols, death, elective, acute,
          ms_col, cdm_col, los_col, age_col, psi_name='PSI19'):
    """
    This function computes the Patient Safety Indicator 19, obstetric trauma,
    ...
    # DELOCMB diagnoses wity any VAGDELP procedures
    delo_diag = df[d_cols].isin(DELOCMD).any(1)
    vag_diag = df[p_cols].isin(VAGDELP).any(1)
    psi_denom = pd.eval('delo_diag & vag_diag')
    ...
    # INSTRIP procedures
    psi_denom.loc[df[p_cols].isin(INSTRIP).any(1)] = False
    # TODO: Missings
    ...
    # denominator with any OBTRAID diagnoses
