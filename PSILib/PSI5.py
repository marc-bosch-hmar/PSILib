...
def PSI5(df, d_cols, p_cols, death, elective, acute,
         ms_col, cdm_col, los_col, age_col, psi_name='PSI5'):
    """
    ...
    psi_num = df[d_cols[1:]].isin(FOREIID).any(1)
    psi_num = df.eval('@psi_num &((' + age_col + '>=18) | (' +
                      cdm_col + ' == "14"))')
    ...
    df.eval(psi_name + ' = 1.0*(@psi_num)', inplace=True)
    return
