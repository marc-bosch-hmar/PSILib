from .Codigos_PSI import MEDIC2R, SURGI2R, FOREIID


def PSI5(df, d_cols, p_cols, death, elective, acute,
         ms_col, cdm_col, los_col, age_col, psi_name='PSI05'):
    """
    This function computes the Patient Safety Indicator 5, retained surgical
    item or unretrieved device fragment.
    ...
    # with any secondary FOREIID diagnosis
    psi_num = df.eval(f"({age_col} >= 18) | ({cdm_col} == '14')")
    psi_num = psi_num & df[ms_col].isin(MEDIC2R + SURGI2R)
    psi_num = psi_num & df[d_cols[1:]].isin(FOREIID).any(1)

    #
    # Numerator exclusion
    #

    # Principal diagnosis for FOREIID or secondary POA
    # TODO: POA
    psi_num.loc[df[d_cols[0]].isin(FOREIID)] = False
    # TODO: Missings
