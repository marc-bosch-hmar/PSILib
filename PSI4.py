...
def PSI4(df, d_cols, p_cols, death, elective, acute,
         ms_col, cdm_col, los_col, age_col, psi_name='PSI04'):
    """
    ...
    expr = f'({age_col}>=18) & ({age_col}<=89) & ('
    if "==" in elective:
        expr += elective + ')'
    else:
        expr += elective + ' == 1)'
    cond_incl = df.eval(f"({expr}) | {cdm_col} == '14'")  # Or MDC 14
    cond_incl = df[ms_col].isin(SURGI2R) & cond_incl  # Surgical discharges

    # STRATUM_DVT: Any secondary FTR2DXB diagnosis
    psi_denom_dvt = df[d_cols[1:]].isin(FTR2DXB).any(1)
    # STRATUM_PNEUMONIA: Any secondary FTR3DX diagnosis
    psi_denom_pneum = df[d_cols[1:]].isin(FTR3DX).any(1)
    # STRATUM_SEPSIS: Any secondary FTR4DX diagnosis
    psi_denom_sepsis = df[d_cols[1:]].isin(FTR4DX).any(1)
    # STRATUM_SHOCK: Any secondary FTR5DX diagnosis or any FTR5PR procedure
    psi_denom_shock = df[d_cols[1:]].isin(FTR5DX).any(1) | \
        df[p_cols].isin(FTR5PR).any(1)
    # STRATUM_GI_HEM: Any secondary FTR6DX diagnosis
    psi_denom_hem = df[d_cols[1:]].isin(FTR6DX).any(1)

    # Aplicamos la condición de inclusión general
    psi_denom_dvt = cond_incl & psi_denom_dvt
    psi_denom_pneum = cond_incl & psi_denom_pneum
    psi_denom_sepsis = cond_incl & psi_denom_sepsis
    psi_denom_shock = cond_incl & psi_denom_shock
    psi_denom_hem = cond_incl & psi_denom_hem
    ...
    if "==" in acute:
        ...
        expr = acute + " == 1"
    cond_excl = df.eval(expr)
    # TODO: Admitted from hospice care
    # TODO: Missings
    # STRATUM_DVT: Principal diagnosis for FTR2DXB or OBEMBOL.
    psi_denom_dvt.loc[df[d_cols[0]].isin(FTR2DXB + OBEMBOL)] = False
    # STRATUM_PNEUMONIA: Principal diagnosis for FTR3DX or FTR3EXA
    psi_denom_pneum.loc[df[d_cols[0]].isin(FTR3DX + FTR3EXA)] = False
    # STRATUM_PNEUMONIA: Any diagnosis for FTR3EXB
    psi_denom_pneum.loc[df[d_cols].isin(FTR3EXB).any(1)] = False
    # STRATUM_PNEUMONIA: Any procedure for LUNGCIP
    psi_denom_pneum.loc[df[p_cols].isin(LUNGCIP).any(1)] = False
    # STRATUM_SEPSIS: Principal diagnosis for FTR4DX or INFECID
    psi_denom_sepsis.loc[df[d_cols[0]].isin(FTR4DX + INFECID)] = False
    # STRATUM_SHOCK: Principal diagnosis for FTR5DX, TRAUMID, HEMORID, GASTRID
    #                or FTR5EX
    psi_denom_shock.loc[df[d_cols[0]].isin(FTR5DX + TRAUMID + HEMORID +
                                           GASTRID + FTR5EX)] = False
    # STRATUM_SHOCK: MCD 4 or 5
    psi_denom_shock.loc[df.eval(cdm_col + ' in ["04", "05"]')] = False
    # STRATUM_GI_HEM: Principal diagnosis for FTR6DX, TRAUMID, ALCHLSM, FTR6EX
    psi_denom_hem.loc[df[d_cols[0]].isin(FTR6DX + TRAUMID + ALCHLSM +
                                         FTR6EX)] = False
    ...
    psi_denom_hem.loc[pd.eval('gv & qd')] = False
    # STRATUM_GI_HEM: MCD 6 or 7
    psi_denom_hem.loc[df.eval(cdm_col + ' in ["06", "07"]')] = False

    # Common exclusions
    psi_denom_dvt[cond_excl] = False
    psi_denom_pneum[cond_excl] = False
    psi_denom_sepsis[cond_excl] = False
    psi_denom_shock[cond_excl] = False
    psi_denom_hem[cond_excl] = False

    # Mutual exclusivity
    stratum = pd.concat(
        [psi_denom_shock, psi_denom_sepsis, psi_denom_pneum, psi_denom_dvt,
         psi_denom_hem],
        axis=1)
    stratum.columns = ["shock", "sepsis", "pneum", "dvt", "hem"]

    stratum.loc[stratum["shock"], ["sepsis", "pneum", "dvt", "hem"]] = False
    stratum.loc[stratum["sepsis"], ["pneum", "dvt", "hem"]] = False
    stratum.loc[stratum["pneum"], ["dvt", "hem"]] = False
    stratum.loc[stratum["dvt"], "hem"] = False

    psi_denom = stratum.any(1)
