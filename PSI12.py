...
def PSI12(df, d_cols, p_cols, death, elective, acute,
    ...
    psi_denom = df.eval(f"({age_col} >= 18) & ({ms_col} in @SURGI2R)")
    psi_denom = psi_denom & df[p_cols].isin(ORPROC).sum(1)
    ...
    orproc_vena = list(set(VENACIP).intersection(set(ORPROC)))
    orproc_vena_sum = df[p_cols].isin(orproc_vena).sum(1)
    psi_denom.loc[orproc_vena_sum == 1] = False
    ...
    orproc_trho = list(set(THROMP).intersection(set(ORPROC)))
    orproc_trho_sum = df[p_cols].isin(orproc_trho).sum(1)
    psi_denom.loc[orproc_trho_sum == 1] = False
