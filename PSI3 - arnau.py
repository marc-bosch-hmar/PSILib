from .listas_formatos import BURNDX, EXFOLIATXD, PIRELBOWD, PILELBOWD, PIRUPBACKD, PILUPBACKD, PIRLOBACKD, PILLOBACKD, PISACRALD, PIRHIPD, PILHIPD, PIRBUTTD, PILBUTTD, PICONTIGBBHD, PIRANKLED, PILANKLED, PIRHEELD, PILHEELD, PIHEADD, PIOTHERD, PINELBOWD, PINBACKD, PINHIPD, PINBUTTD, PINANKLED, PINHEELD, PIUNSPECD, PICONTBBHEXD, PIHEADEXD, PILANKLEXD, PILBUTEXD, PILELBOEXD, PILHEELEXD, PILHIPEXD, PILLOBACEXD, PILUPBACEXD, PIOTHEREXD, PIRANKLEXD, PIRBUTEXD, PIRELBOEXD, PIRHEELEXD, PIRHIPEXD, PIRLOBACEXD, PIRUPBACEXD, PISACRAEXD

PIEXD = ['PICONTBBHEXD', 'PIHEADEXD', 'PILANKLEXD', 'PILBUTEXD', 'PILELBOEXD', 
 'PILHEELEXD', 'PILHIPEXD', 'PILLOBACEXD', 'PILUPBACEXD', 'PIOTHEREXD',
 'PIRANKLEXD', 'PIRBUTEXD', 'PIRELBOEXD', 'PIRHEELEXD', 'PIRHIPEXD',
 'PIRLOBACEXD', 'PIRUPBACEXD', 'PISACRAEXD']
 
PID = ["PIRELBOWD", "PILELBOWD", "PIRUPBACKD", "PILUPBACKD", 
    "PIRLOBACKD", "PILLOBACKD", "PISACRALD", "PIRHIPD", "PILHIPD", "PIRBUTTD", "PILBUTTD", "PICONTIGBBHD", 
    "PIRANKLED", "PILANKLED", "PIRHEELD", "PILHEELD", "PIHEADD", "PIOTHERD", "PINELBOWD", "PINBACKD", "PINHIPD", 
    "PINBUTTD", "PINANKLED", "PINHEELD", "PIUNSPECD"]
...
def PSI3(df, d_cols, p_cols, death, elective, acute,
    ...
    psi_denom.loc[df[d_cols[0]].isin(PIEXD)] = False #Totes les agrup de diag que comencen per PI i acaben per EXD.
    ...
    #Newborns & Other Neonates with Conditions Originating in Perinatal Period
    psi_denom.loc[df.eval(cdm_col + ' == "15"')] = False
    # TODO: Missings
    ...
    psi_num = df[d_cols[1:]].isin(PID).any(1) #que hi hagi qualsevol D secundari d'aquests.
    psi_num.loc[df[poa_cols].isin(PID).any(1)] = False #Si hi ha un D d'aquests que sigui POA, ho exclou
    psi_num.loc[df[d_cols[1:]].isin(DTIEXD).any(1)] = False #Si hi ha un D secundari d'aquests, ho exclou
    psi_num.loc[~df[poa_cols].isin(DTIEXD).any(1)] = False #Si NO hi ha un D d'aquests que sigui POA, ho exclou
